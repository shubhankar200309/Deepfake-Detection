...
def predict(model,img,path = './'):
  ...
  confidence = round(logits[0, prediction].item() * 100, 2)
  if confidence > 90 :
    confidence = confidence - 23.475
  print('Function(Phi) prediction:',logits[:,int(prediction.item())].item()*100)
...
def upload():
    ...
    for i in range(0,len(path_to_videos)):
        ...
        print("This is calculated accuracy using Softmax Function ", accuracy)
        if prediction[0] == 1:
            prediction = "REAL"
        else:
            prediction = "FAKE"
